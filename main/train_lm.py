...
def main():
    ...
    def group_texts(examples, tokenizer, block_size=512):
        ...
        for ids in examples['input_ids']:
            ...
            # 复制 input_ids 到 labels，并确保标签也被填充/截断
            batch['labels'].append(truncated_ids + [tokenizer.pad_token_id] * padding_length)

        return batch

    # 使用 batched=True 和正确的填充逻辑
    with training_args.main_process_first(desc="grouping texts together"):
        if not data_args.streaming:
            lm_datasets = tokenized_datasets.map(
                lambda examples: group_texts(examples, tokenizer, block_size),
                batched=True,  # 重要：确保使用批量处理
                num_proc=data_args.preprocessing_num_workers,
                load_from_cache_file=not data_args.overwrite_cache,
                desc=f"Grouping and padding texts in chunks of {block_size}",
                batch_size=40000,  # 根据需要调整
            )
        else:
            lm_datasets = tokenized_datasets.map(
                lambda examples: group_texts(examples, tokenizer, block_size),
                batched=True,  # 重要：确保使用批量处理
                batch_size=60000,  # 根据需要调整
            )
