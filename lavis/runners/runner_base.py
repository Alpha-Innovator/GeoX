...
from lavis.builder_dataloader import (
    IterLoader,
    MultiIterLoader,
    PrefetchLoader,
)
...
class RunnerBase:
    ...
    def lr_scheduler(self):
        ...
        if self._lr_sched is None:
            ...
            warmup_epoch = self.config.run_cfg.get("warmup_epoch", 5000)
    ...
    @property
    def save_freq(self):
        save_freq = self.config.run_cfg.get("save_freq", 100)
        return int(save_freq)

    @property
    def val_freq(self):
        val_freq = self.config.run_cfg.get("val_freq", 200)
        return int(val_freq)
