...
import torch
from configs.models.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_SEP_TOKEN

from torchvision import transforms
from PIL import Image
class ImageProcessor:
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], mode='train', image_size=224, interpolation=3, min_scale=0.2, max_scale=1.0):
        self.mode = mode
        # 设置转换
        if mode == 'train':
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            from models.geox.mm_llama import LlavaConfig
