...
class GeometryEvaluator:
    ...
    def process_file(self, json_file, pk_file):
        ...
        # 
        if 'calculation' in pk_file:
            problem_form = 'calculation'
        elif 'proving' in pk_file:
            problem_form = 'proving'
        else:
            raise ValueError("File name does not specify problem type (calculation or proving)")
        ...
        for result in json_data:
            ...
            elif problem_form == 'proving':
                self.geo_evaluation_prove(result['text'], corresponding_data)
    ...
    def geo_evaluation_prove(self, preds, batch):
        problem_type = batch['problem_type']
        target = ' '.join(batch['proving_sequence'])

        # Assume preds is a list of top-k predictions from beam search
        success = None
        for pred in preds:
            success = self.evaluate_proving(pred, target)
            if success:
                break
    ...
    def evaluate_proving(self, prediction, target):
        hypo = prediction.split()
        if hypo == target.split():
            return True
        return None
    ...
    def print_results(self):
        ...
        proving_metrics = [
            ("Accuracy", self.proving_acc.get_avg() * 100),
            ("No Result", self.proving_no_result.get_avg() * 100),
            ("Parallel", self.prove_parallel.get_avg() * 100),
            ("Triangle", self.prove_triangle.get_avg() * 100),
            ("Quadrilateral", self.prove_quadrilateral.get_avg() * 100),
            ("Congruent", self.prove_congruent.get_avg() * 100),
            ("Similarity", self.prove_similarity.get_avg() * 100)
        ]

        # Determine the maximum length of metric names to ensure uniform table width
        max_len = max(len(metric[0]) for metric in calculation_metrics + proving_metrics)
        ...
        proving_table = format_table("Proving", proving_metrics)
        # Ensuring both tables are the same width
        max_table_width = max(len(calculation_table.split('\n')[0]), len(proving_table.split('\n')[0]))
        print(calculation_table)
        print(proving_table)
        return self.calculation_acc.get_avg() * 100, self.proving_acc.get_avg() * 100
...
def main():
    ...
    parser.add_argument('--cal_gt_file', type=str, default="./data/unigeo/calculation_test.pk",
                        help='Path to the calculation PK file.')
    parser.add_argument('--prov_pred_file', type=str, required=True,
                        help='Path to the proving results JSONL file.')
    parser.add_argument('--prov_gt_file', type=str, default="./data/unigeo/proving_test.pk",
                        help='Path to the proving PK file.')
    ...
    evaluator.process_file(args.prov_pred_file, args.prov_gt_file)
